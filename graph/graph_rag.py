...
from graph.graph_manager import GraphManager, Triple, get_graph_manager
...
class GraphRAG:
    ...
    def search(
        ...
        if self.enable_graph and self.graph_manager and self.graph_manager.connected:
            ...
            if entities:
                # 将权限级别转换为来源前缀
                # 例如: ["public", "internal"] -> ["public/", "internal/"]
                allowed_sources = None
                if allowed_levels:
                    allowed_sources = [f"{level}/" for level in allowed_levels]

                # 图谱检索（带权限过滤）
                graph_result = self._retrieve_from_graph(entities, graph_depth, allowed_sources)
...
实体："""
    ...
    def _retrieve_from_graph(
        ...
        Args:
            entities: 实体名称列表
            depth: 搜索深度
            allowed_sources: 允许访问的来源文件前缀列表（用于权限过滤）
        ...
        # 搜索子图（带权限过滤）
        subgraph = self.graph_manager.search_subgraph(entities, depth, allowed_sources)
