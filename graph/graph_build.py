...
# 导入图谱管理器（同目录）
from graph.graph_manager import GraphManager, Entity, Triple
...
def extract_document(filepath: str) -> List[Dict]:
    ...
    # 获取相对路径（包含权限目录，如 public/文件.txt 或 confidential/文件.docx）
    # 用于权限控制
    try:
        # 尝试获取相对于 documents 目录的相对路径
        docs_path = DOCUMENTS_PATH
        if filepath.startswith(docs_path):
            relative_path = os.path.relpath(filepath, docs_path)
            # 统一使用正斜杠，如 "confidential/薪酬制度.docx"
    ...
    if ext == '.txt':
        ...
        for i, chunk in enumerate(split_text(text)):
            chunks.append({'content': chunk, 'source': relative_path, 'chunk': i})
...
def build_graph_from_files(filepaths: List[str], clear_existing: bool = False, verbose: bool = True) -> int:
    ...
    success = gm.build_from_triples(unique_triples)
