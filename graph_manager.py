...
class GraphManager:
    ...
    def build_from_triples(
        ...
        for triple in triples:
            entities.add((triple.head.name, triple.head.type))
            entities.add((triple.tail.name, triple.tail.type))

        print(f"正在创建 {len(entities)} 个实体...")
        for name, entity_type in entities:
            self.create_entity(name, entity_type)
    ...
    def search_subgraph(
        ...
        RETURN DISTINCT
            n.name as start_name, labels(n) as start_types,
            m.name as end_name, labels(m) as end_types,
            ...
            with self.driver.session() as session:
                ...
                for record in result:
                    ...
                    if start_name not in nodes:
                        nodes[start_name] = {'name': start_name, 'types': start_types}
                    if end_name not in nodes:
                        nodes[end_name] = {'name': end_name, 'types': end_types}
